...
class GCSMessages(object):
    ...
    def __init__(self, interface):
        ...
        self.__timeout = 500  # milliseconds
    ...
    def readPN(self, tosend):
        if len(tosend) > 1:
            tosend += '\n'
        self.__interface.sendPN(tosend)
        chunks = []
        while True:
            if self.__interface.answersizePN:
                received = self.__interface.getanswerPN(self.__interface.answersizePN)
                chunks.append(received)
                if endofanswerPN(chunks[-1]):
                    break
        return ''.join(chunks)
