...
class GCSDll(PIGateway):
    ...
    def __init__(self, devname='', dllname=''):
        ...
        self.__dllhandlePN = ctypes.windll.LoadLibrary(self.__dllpath)
        self.__id = -1
        self.__dcid = -1
        self.__ifdescription = ''
        self.__asyncbufferindex = -1  # DEPRECATED
        self.__size = ctypes.c_int()
    ...
    def sendPN(self, command):
        command = ctypes.c_char_p(command.encode())
        getattr(self.__dllhandlePN, 'PI_GcsCommandset')(self.__id, command)
    ...
    @property
    def answersizePN(self):
        size = ctypes.c_int()
        getattr(self.__dllhandlePN, 'PI_GcsGetAnswerSize')(self.__id, ctypes.byref(size))
        return size.value
    ...
    def getanswerPN(self, bufsize):
        bufstr = ctypes.create_string_bufferPN('\000'.encode(), bufsize + 2)
        getattr(self.__dllhandlePN, 'PI_GcsGetAnswer')(self.__id, bufstr, bufsize + 1)
        return bufstr.value.decode(encoding='cp1252')
